...
class chatsmodel extends CI_Model {
	...
	{
		$this->db->insert('addproductchats', $data);
	...
	{
		$this->db->where('id', $id);
		$this->db->delete('addproductchats');
		return $this->db->affected_rows()!=1 ? false:true;
	}

	public function billadd($data)
	{
		$this->db->insert('addchats', $data);
		return $this->db->affected_rows()!=1 ? false:true;
	}

		public function search_chats()
	{
		   

		if($this->session->userdata('chats_fromdate')!="")
		{
			$fromdate=date('Y-m-d',strtotime(str_replace('/','-',$this->session->userdata('chats_fromdate'))));   
		}
		else
		{
			$fromdate='';
		}
		if($this->session->userdata('chats_todate')!="")
		{
			$todate=date('Y-m-d',strtotime(str_replace('/','-',$this->session->userdata('chats_todate'))));
		}
		else
		{
			$todate='';
		...
		if(@$this->session->userdata('chats_billno'))
		{


			$this->db->where('billno',$this->session->userdata('chats_billno'));
		}



		return $query=$this->db->get('addchats')->result_array();
