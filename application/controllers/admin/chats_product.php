...
class Chats_product extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('admin/chatsproductmodel','person');
	...
	public function index()
	{
			...
			$this->load->view('admin/chats_product');
		...
		public function checkproductname()
	{
		...
		$data=$this->db->where('productname',$productname)->get('chatsproduct')->result_array();
		//echo $category;
		if($data)
		{
			echo 'Yes';
	...
	public function addproductid()
	{
		...
		$query_update1 =mysql_query("SELECT * FROM chatsproduct ORDER BY id DESC LIMIT 1");
	...
	@$bond_no = $productid;
        if(is_null($bond_no)){
          ...
          if($bond_length == 1){
            $new_bond_noo = 'CR-00'.$va;  
          } 
          else if($bond_length == 2)
          {  
            $new_bond_noo = 'CR-0'.$va; 
          }
