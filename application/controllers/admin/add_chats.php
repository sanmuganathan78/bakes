...
class Add_chats extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('admin/chatsmodel');
     $this->load->model('admin/chatsmodel','person');
	...
	public function index()
	{
            ...
            $query_update1 =mysql_query("SELECT * FROM addchats ORDER BY id DESC LIMIT 1");
    ...
    @$bond_no = $billno;
        if(is_null($bond_no)){
          ...
          if($bond_length == 1){
            $new_bond_noo = 'CN-0000'.$va;  
          } 
          else if($bond_length == 2)
          {  
            $new_bond_noo = 'CN-000'.$va; 
          }
          else if($bond_length == 3)
          {  
            $new_bond_noo = 'CN-00'.$va; 
          }
          else if($bond_length == 3)
          {  
            $new_bond_noo = 'CN-0'.$va; 
          }
          else if($bond_length == 3)
          {  
            $new_bond_noo = 'CN-'.$va; 
        ...
        $data['billnos']=$new_bond_noo;
			...
			$this->load->view('admin/add_chats',$data);
        ...
        public function searchitem()
  {
    ...
    $result=$this->db->where('status',1)->like('productname',$productname)->get('chatsproduct')->result();
    ...
    public function searchrate()
  {
    ...
    $data=$this->db->where('status',1)->where('productname',$productname)->get('chatsproduct')->result_array();
   ...
   @$bond_no = $billno;
        ...
        {   
                ...
                $data=array('date'=>date('Y-m-d'),
                            ...
                            'status'=>1);
                $result=$this->chatsmodel->billadd($data);
                if($result==true)
                {
                ...
                $this->db->insert('chatitemizedstock',$dataas);
                $strs = $this->db->last_query();
                    ...
                    redirect('admin/add_chats/print_bill');

                }
                else
                {
                    $this->session->set_flashdata('msg1','Bill added Failed');
                    redirect('admin/add_chats');
                }
    ...
    {
        $data['printsdata']=$this->db->order_by('id','DESC')->limit('1')->get('addchats')->result_array();
       $this->load->view('admin/chatsprintbill',$data);
    }


    public function chats_reports()
    {
        $data['chatsreports']=$this->db->where('status',1)->get('addchats')->result_array();
        $this->load->view('admin/chats_reports',$data);
    }


     public function chats_report()
    {
            $data['chatsreports']=$this->chatsmodel->search_chats();
     ...
     public function billing_reportprints()
              { 
               ...
               $data=array(
                        'chats_fromdate' => $fromdate,
                        'chats_todate' => $todate,
                        'chats_billno' => $billno,
                        'chats_bill_format' =>'Bill_Print',
                       );
              ...
              { 
               ...
               $data=array(
                        ...
                        'chats_todate' => $todate,
                        'chats_billno' => $billno,
                        'chats_bill_format' =>'Bill_Download',
                       );
               $this->session->set_userdata($data);
    ...
    public function chats_reportprint()
    {   
               $bill_format=$this->session->userdata('chats_bill_format');                
                
                if($bill_format=='Bill_Print')
                {
                  $data['chatsreports']=$this->chatsmodel->search_chats();
                  $data['fromdate']=$this->session->userdata('chats_fromdate');
                  $data['todate']=$this->session->userdata('chats_todate');

                   $this->load->view('admin/chats_reportprint',$data);
                ...
                elseif($bill_format=='Bill_Download')
                {
                  ...
                  $chatsreports=$this->chatsmodel->search_chats();
                  $fromdate=$this->session->userdata('chats_fromdate');
                  $todate=$this->session->userdata('chats_todate');
...
<table width="724" border="0" align="center" style="border-collapse:collapse; font-family: Arial, Helvetica, sans-serif;">
  <tr style="font-size: 14px;">
    <td height="35" width="222" align="left" style="border-bottom:1px solid black;"><strong>Chats   Report</strong></td>
   ...
   <tr>
    ...
    <td width="186" align="left" style="border-bottom:1px solid black;"><strong></strong></td>
     ...
     <td width="112" align="right" style="border-bottom:1px solid black;"><strong>'. number_format(array_sum($totalamount),2).'</strong></td>
                  ...
                  $mpdf->WriteHTML($html);  
                    $filename=date('Y-m-d-').'Chats Reports.pdf';
   ...
   public function viewbilling()
      {
        ...
        $data=$this->db->where('id',$id)->get('addchats')->result_array();
        if($data)
        {
          foreach ($data as $row) {
